#!/usr/bin/env python3
"""
Multi-GPU Subject Completeness Detection using vLLM

This script processes JSONL files containing image generation results and detects
whether the generated images contain complete subjects using Qwen2.5-VL model.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Detect subject completeness in generated images using multi-GPU processing",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Process single file
  python vllm_subject_mp.py --input_file output/flux/merged_result_flux.jsonl
  
  # Process multiple files
  python vllm_subject_mp.py --input_files output/flux/merged_result_flux.jsonl output/sd3/merged_result_sd3.jsonl
  
  # Use custom model and settings
  python vllm_subject_mp.py --input_file data.jsonl --model_path /path/to/model --batch_size 16 --world_size 4
        """
    )
    ...
    input_group.add_argument('--input_file', type=str,
                           help='Single JSONL file containing prompts and images')
    input_group.add_argument('--input_files', nargs='+', 
                           help='Multiple JSONL files containing prompts and images')
    
    # Model and processing options
    parser.add_argument('--model_path', type=str, default=DEFAULT_MODEL_PATH,
                       help=f'Path to the Qwen2.5-VL model (default: {DEFAULT_MODEL_PATH})')
...
def setup_distributed(rank: int, world_size: int, port: str = '12357') -> None:
...
def create_detection_prompt(prompt_t2i: str, subject: str) -> str:
    """
    Create prompt for subject completeness detection.
    
    Args:
        prompt_t2i (str): Original text-to-image prompt
        subject (str): Subject to detect
        
    Returns:
        str: Detection prompt
    """
    return f"""
This is an image generated by the prompt '{prompt_t2i}'. Please determine whether it contains a complete {subject} according to the following criteria:

1. If the {subject} is a person:
- At least one hand or foot (i.e., one limb) must be clearly visible in its entirety. If none of the hands, feet, arms, or legs are clearly shown in the image, consider it "incomplete".
- A complete hand must be visible. If the hand is not shown, is obscured, or is outside the frame, it should also be considered "incomplete". If only the head or upper body is shown, or if no complete limbs are visible, consider it "incomplete".

2. If the {subject} is an animal:
- At least one limb must be clearly visible in its entirety. If no limb is clearly shown in the image, consider it "incomplete".

3. If the {subject} is an object or other category:
- Simply determine whether the complete {subject} is present in the image. If it is complete, consider it "complete".

4. If the image is blurry or unclear, consider it "incomplete".

First, briefly explain the reason for your judgment. Then, on a separate line, provide your final answer with only "Yes" or "No".
Please strictly follow the criteria above when making your judgment.
"""

def process_batch_data(batch_data: List[Dict], processor, llm, sampling_params, verbose: bool = False) -> List[Dict]:
    """
    Process a batch of data for subject detection.
    ...
    Returns:
        List[Dict]: Processed batch data with detection results
    """
    batch_inputs = []
    ...
    for data in batch_data:
        ...
        subject = data.get("subject", "subject")  # Default to "subject" if not specified
        ...
        detection_prompt = create_detection_prompt(prompt_t2i, subject)
        
        # Prepare content based on image availability
        if image_path is None or not os.path.exists(image_path):
            content = [{"type": "text", "text": detection_prompt}]
            if verbose:
    ...
    for j, output in enumerate(outputs):
        ...
        # Extract "Yes" or "No" judgment
        has_subject = "Yes" in generated_text[-10:]  # Check last 10 characters for "Yes"
        
        # Add results to data
        batch_data[j]["has_subject"] = has_subject
        batch_data[j]["subject_detection_response"] = generated_text
    
    return batch_data

def detect_subject_completeness(rank: int, world_size: int, args: argparse.Namespace) -> None:
    """
    ...
    for input_file in args.input_files:
        ...
        # e.g., "output/flux/merged_result_flux.jsonl" -> "flux"
        ...
        output_file = os.path.join(output_dir, f"subject_detection_{model_name}.jsonl")
...
def main():
    ...
    if world_size > 1:
        ...
        mp.spawn(
            detect_subject_completeness,
            args=(world_size, args),
            nprocs=world_size,
            join=True
        )
    else:
        detect_subject_completeness(0, world_size, args)
